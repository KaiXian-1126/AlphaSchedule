...
class _DrawerScreenState extends State<DrawerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: calendarDependency.getCalendarList(user: widget.user),
      builder: (context, snapshot) {
        if (calendarList != null ||
            collaboratorCalendarList != null ||
            eventList != null) {
          print(
              "calendar name: ${calendarList[currentCalendarIndex].calendarName}");
        ...
        onTap: (index) async {
          ...
          if (index == 1) {
            ...
            final response = await Navigator.pushNamed(
                ...
                arguments: [calendarList[currentCalendarIndex], widget.user]);
          } else if (index == 3) {
            final response = await Navigator.pushNamed(
            ...
            Navigator.pushNamed(context, eventSearchRoute,
                arguments: calendarList[currentCalendarIndex].eventList);
          } else if (index == 5) {
            final response = await Navigator.pushNamed(
